...
const Auth: React.FC = () => {
  ...
  const signUpEmail = async () => {
      // 作られたユーザーを返すのでauthUserにユーザオブジェクトを格納
      const authUser = await auth.createUserWithEmailAndPassword(email, password);
      let url = "";
      if (avatarImage) { // avatarImageが存在する場合は保存する
        const S =
        ...
        await storage.ref(`avatars/${fileName}`).put(avatarImage); // putの引数にファイルの実体を入れる
      ...
      <Grid item xs={12} sm={8} md={5} component={Paper} elevation={6} square>
        <div className={classes.paper}>
          ...
          {!isLogin && (
            ...
            <Grid container>
                <Grid item xs>
                    <span className={styles.login_reset}>パスワードを忘れた方はこちら</span>
                </Grid>
                <Grid item>
                    <span 
